...
- **Verificación Canónica de URLs Oficiales (Cero Subdominios Temporales)**: Queda estrictamente prohibido emitir o registrar URLs temporales autogeneradas por Vercel o CDNs (ej. sufijos aleatorios). La única fuente de verdad absoluta para la URL de producción es el archivo de acceso directo `.url` y la ficha técnica en `C:\Users\mauro\OneDrive\Desktop\Otros Prototipos\`. Para Ataraxia, la única URL oficial es: `https://ataraxia-stoic.vercel.app`.
